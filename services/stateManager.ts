...
export enum KEEP_ALIVE_STATUS {
  ON = "ON",
  OFF = "OFF",
}
class StateManager {
  getSchedule():
    | { user: string; startupTime: Date; shutdownTime: Date }
    | undefined {
    try {
  ...
  saveSchedule(user: string, startupTime: Date, shutdownTime: Date) {
    try {
      fs.appendFileSync(
        "./states/uptimeSchedule.csv",
        `${user};${startupTime.toISOString()};${shutdownTime.toISOString()}\n`
      );
    } catch (error) {
      console.error(error);
    }
  ...
  getKeepAliveStatus(): KEEP_ALIVE_STATUS {
    try {
      const state = fs.readFileSync("./states/keepAlive.csv", "utf-8");
      if (!state) {
